...
// ==========================================================================================
// 傳送門之間是 queue 可循環讓內部字典序最小
// 左右兩邊是 stack 可互相丟 先丟左再丟右
// unAC
// ==========================================================================================

void solve()
{
    int n, x, y, t; cin >> n >> x >> y;
    string s;
    ...
    string q = s.substr(x,y-x), m = q;
    RPT(i,q.size())
    {
        q = q.substr(1,q.size()) + q[0];
        if(q < m) m = q;
    }

    string l = s.substr(0,max(x,0LL)), r = s.substr(min(y,n),n);
    while(!m.empty() && !l.empty() && m.front() < l.back())
    {
        r = l.back() + r;
        l.pop_back();
    }
    
    while(!m.empty() && !r.empty() && m.front() > r.front())
    {
        l += r.front();
        r = r.substr(1);
    }
