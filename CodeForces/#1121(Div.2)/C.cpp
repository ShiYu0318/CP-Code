...
{
    int n; cin >> n;
    vi v(n); input(v); sortall(v);
    int fact = 1, suf = 0, ans = 0;
    FOR(i, 1, n) fact = fact * i % MOD;
    rFOR(i, n-1)
    {
        suf = (suf + v[i+1]) % MOD; // S_i = a[i+1] + ... + a[n-1]
        int k = n-1-i;   // 比 a[i] 大的人數
        int term = ((suf - k % MOD * v[i]) % MOD + MOD) % MOD;
        ans = (ans + term * inv[k]) % MOD;
    }
    cout << ans * fact % MOD << '\n';
}
...
signed main()
{
    ShiYu;
    inv[1] = 1;
    FOR(i, 2, MAXN) inv[i] = (MOD - MOD/i) * inv[MOD%i] % MOD;
    int t = 1;
