...
void solve()
{
    ...
    for(auto &v : a) cin >> v;
    vector<unsigned> ans;
    vector<unsigned> cur = a, buf;
    buf.reserve((size_t)n * (n-1) / 2);
    while(true) 
    {
        ans.EB(*max_element(all(cur))
             - *min_element(all(cur)));
        if(ans.back() == 0) break;
        buf.clear();
        RPT(i,n) for(int j = i+1; j < n; j++) buf.EB(cur[i] ^ cur[j]);
        nth_element(buf.begin(), buf.begin() + n, buf.end());
        copy(buf.begin(), buf.begin() + n, cur.begin());
    }
    while(q--) 
    {
        unsigned x; cin >> x;
        cout << (x < ans.size() ? ans[x] : 0u) << '\n';
    }
}
