...
def bfs_path_with_limits(start, target, R, TTL):
    """BFS，每一步距離 <= R，步數 <= TTL"""
    parent = [-1]*N
    dis = [-1]*N
    dis[start] = 0
...
def update():
    ...
    if op == 1:
        ...
        if path is None:
            ...
            partial_path = bfs_path_with_limits(a, b, float('inf'), TTL)
            if partial_path:
                draw_path(ax, partial_path, color="red")
                zoom_nodes = partial_path
        else:
